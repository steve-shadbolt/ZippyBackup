...
namespace Ionic.Zip
{
    public partial class ZipEntry
    {
        ...
        class DiagnosticStream : Stream
        {
            string Label;
            Stream Underlying;
            TextWriter Diagnostic;

            public DiagnosticStream(Stream Underlying, TextWriter DiagWriter, string Label)
            {
                this.Underlying = Underlying;
                this.Diagnostic = DiagWriter;
                this.Label = Label;
            }
            ...
            {
                if (Diagnostic != null) Diagnostic.WriteLine("DiagnosticStream.Write: before " + Label + "...");
                Underlying.Write(buffer, offset, count);
                if (Diagnostic != null) Diagnostic.WriteLine("DiagnosticStream.Write: after " + Label + "...");
            }

            public override void WriteByte(byte value)
            {
                if (Diagnostic != null) Diagnostic.WriteLine("DiagnosticStream.Write: before byte " + Label + "...");
                Underlying.WriteByte(value);
                if (Diagnostic != null) Diagnostic.WriteLine("DiagnosticStream.Write: after byte " + Label + "...");
            }
        }
        ...
        private void _WriteEntryData(Stream s)
        {
            ...
            try
            {
                ...
                if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): establishing layers...");

if (Diagnostic != null) s = new DiagnosticStream(s, Diagnostic, "Base-Stream");

                CountingStream entryCounter = new CountingStream(s);
                Stream counter = entryCounter;

if (Diagnostic != null) counter = new DiagnosticStream(entryCounter, Diagnostic, "Counter");
                ...
                if (fileLength != 0L)
                {
                    ...
                    // is used.
                    encryptor = MaybeApplyEncryption(counter);
                ...
                {
                    encryptor = compressor = counter;
                }

if (Diagnostic != null) compressor = new DiagnosticStream(compressor, Diagnostic, "Compressor");

                // Wrap a CrcCalculatorStream around that.
                // This will happen BEFORE compression (if any) as we write data out.
                var CrcOutput = new Ionic.Crc.CrcCalculatorStream(compressor, true);
                Stream output = CrcOutput;

if (Diagnostic != null) output = new DiagnosticStream(CrcOutput, Diagnostic, "CrcCalculator");
                ...
                else
                {
                    ...
                    int n;
                    if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): starting ReadWithRetry loop...");
                    while ((n = SharedUtilities.ReadWithRetry(input, buffer, 0, buffer.Length, FileName, Diagnostic)) != 0)
                    {
                        if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): after ReadWithRetry...");
                        output.Write(buffer, 0, n);
                        if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): after output.Write()...");
                        OnWriteBlock(CrcOutput.TotalBytesSlurped, fileLength);
                        if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): after OnWriteBlock()...");
                        if (_ioOperationCanceled)
                            break;
                        if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): next ReadWithRetry...");
                    }
                    if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): finished ReadWithRetry loop...");
                }

                if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): calling FinishOutputStream...");
                FinishOutputStream(s, entryCounter, encryptor, compressor, CrcOutput);
            }
            finally
            {
                ...
                {
                    if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): Closing input stream...");
#if NETCF
                    input.Close();
#else
                    input.Dispose();
            ...
            if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData(): Post-processing...");
        ...
        /// <summary>
        /// WB/W.B./Wiley Black Addition:
        /// Set this to a TextWriter in order to feed it more diagnostic trace output when saving.
        /// </summary>
        public TextWriter Diagnostic;


        internal void Write(Stream s)
        {
            if (Diagnostic != null) Diagnostic.WriteLine("Entering ZipEntry.Write.");
            ...
            do
            {
                try
                {
                    ...
                    do
                    {
                        ...
                        if (Diagnostic != null) Diagnostic.WriteLine("WriteHeader()...");
                        WriteHeader(s, nCycles);

                        if (Diagnostic != null) Diagnostic.WriteLine("WriteSecurityMetadata...");
                        // write the encrypted header
                        WriteSecurityMetadata(s);

                        if (Diagnostic != null) Diagnostic.WriteLine("_WriteEntryData...");
                        // write the (potentially compressed, potentially encrypted) file data
                        _WriteEntryData(s);

                        if (Diagnostic != null) Diagnostic.WriteLine("_TotalEntrySize = ...");
                    ...
                    if (Diagnostic != null) Diagnostic.WriteLine("ZipEntry Done!");
                }
                catch (System.Exception exc1)
                {
                    if (Diagnostic != null) Diagnostic.WriteLine("ZipEntry EXCEPTION!");
