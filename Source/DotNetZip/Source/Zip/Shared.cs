...
namespace Ionic.Zip
{
    ...
    internal static class SharedUtilities
    {
        ...
        {
            return ReadWithRetry(s, buffer, offset, count, FileName, null);
        }
        internal static int ReadWithRetry(System.IO.Stream s, byte[] buffer, int offset, int count, string FileName, TextWriter Diagnostic)
        {
            int n = 0;
            bool done = false;
#if !NETCF && !SILVERLIGHT
            ...
            do
            {
                try
                {
                    if (Diagnostic != null) Diagnostic.WriteLine("ReadWithRetry(): About to s.Read()...");
                    n = s.Read(buffer, offset, count);
                    if (Diagnostic != null) Diagnostic.WriteLine("ReadWithRetry(): Done with that s.Read()...");
                    done = true;
                }
#if NETCF || SILVERLIGHT
                ...
                {
                    if (Diagnostic != null) Diagnostic.WriteLine("ReadWithRetry(): Exception: " + ioexc1.ToString());
                    ...
                    if (p.IsUnrestricted())
                    {
                        ...
                        if (Diagnostic != null) Diagnostic.WriteLine("ReadWithRetry(): Exception had hresult = " + hresult);
